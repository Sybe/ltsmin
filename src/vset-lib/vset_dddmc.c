...
struct poptOption dddmc_options[]= {
    { "dddmc-tablesize", 0, POPT_ARG_INT|POPT_ARGFLAG_SHOW_DEFAULT, &datasize , 0 , "log2 initial size of dDD nodes table", "<tablesize>"},
    { "dddmc-maxtablesize", 0, POPT_ARG_INT|POPT_ARGFLAG_SHOW_DEFAULT, &maxtablesize , 0 , "log2 maximum size of dDD nodes table", "<maxtablesize>"},
    { "dddmc-cachesize", 0, POPT_ARG_INT|POPT_ARGFLAG_SHOW_DEFAULT, &cachesize , 0 , "log2 size of memoization cache", "<cachesize>"},
    { "dddmc-maxcachesize", 0, POPT_ARG_INT|POPT_ARGFLAG_SHOW_DEFAULT, &maxcachesize , 0 , "log2 maximum size of memoization cache", "<maxcachesize>"},
    POPT_TABLEEND
};
...
struct vector_set {
    ...
    int discrete_vars;
    int clocks; // dimension of the original DBM
...
set_create(vdom_t dom, int k, int *proj)
{
    ...
    set->discrete_vars = dom->discrete_vars;
    set->dom  = dom;
    set->mdd  = dddmc_false;
    set->size = k < 0 ? dom->shared.size : k;
    set->clocks = (int)sqrt(set->size - set->discrete_vars) + 1;
#if DEBUG_MT
    set->mtctr = 0;
#endif
    ...
    if (k < 0) {
        ...
        _proj[0] = -1;
        set->proj = dddmc_ref(dddmc_cube((uint32_t*)_proj, 1, dom->shared.size+1));//nr of discrete vars not needed for projection
    } else if (k == 0) {
        _proj[0] = -2;
        set->proj = dddmc_ref(dddmc_cube((uint32_t*)_proj, 1, dom->shared.size+1));
    } else {
        ...
        _proj[proj[k-1]+1] = -2;
        set->proj = dddmc_ref(dddmc_cube((uint32_t*)_proj, proj[k-1]+2, dom->shared.size+1));
    }
...
rel_create_rw(vdom_t dom, int r_k, int *r_proj, int w_k, int *w_proj)
{
    ...
    rel->meta = dddmc_ref(dddmc_cube((uint32_t*)meta, j, dom->shared.size*2+1));//don't use the op and type in the relation
...
set_add(vset_t set, const int* e)
{
    ...
    LACE_ME;
    MDD old = set->mdd;
    set->mdd = dddmc_ref(dddmc_union_cube(set->mdd, (uint32_t*)e, set->size, set->discrete_vars));
    dddmc_deref(old);
    leavemt(set);
}
...
rel_add_cpy(vrel_t rel, const int *src, const int *dst, const int *cpy)
{
    ...
    LACE_ME;
    MDD old = rel->mdd;
    rel->mdd = lddmc_ref(dddmc_union_cube_copy(rel->mdd, (uint32_t*)vec, cpy_vec, k));
    lddmc_deref(old);
    leavemt(rel);
}
...
{
    if(set->mdd == dddmc_false) return 1;
    LACE_ME;
    //dddmc_fprintdot(stdout, set->mdd);
    return dddmc_equal(set->mdd, dddmc_false);
//    return (set->mdd == dddmc_false);
}
//TODO bi-implication
static int
set_equal(vset_t set1, vset_t set2)
{
    ...
    if(set1->mdd == set2->mdd){
        return 1;
    }
    LACE_ME;
    int result = CALL(dddmc_equal, set1->mdd, set2->mdd);
    return result;
}
//TODO klaar
static void
...
set_copy(vset_t dst, vset_t src)
{
    ...
    dddmc_deref(dst->mdd);
    dst->mdd = dddmc_ref(src->mdd);
    leavemt(dst);
}
//TODO met subsumption
static int
set_member(vset_t set, const int* e)
{
    return dddmc_member_cube(set->mdd, (uint32_t*)e, set->size);
}
//TODO count van elementen, ook nog beslissen wanneer iets een element is, aantal discrete states?
static void
set_count(vset_t set, long *nodes, bn_int_t *elements)
{
    //Warning(info, "set_count");
    entermt(set);
    LACE_ME;
    //set->mdd = dddmc_reduce(set->mdd, set->clocks);
    if (nodes != NULL) *nodes = dddmc_nodecount(set->mdd);
    if (elements != NULL) bn_double2int(dddmc_satcount_cached(set->mdd), elements);
    int32_t values[set->discrete_vars];
    //dddmc_print_discrete_states(set->mdd, set->discrete_vars, 0, values);
    leavemt(set);
}
...
set_union(vset_t dst, vset_t src)
{
    ...
    assert(dst->size == src->size);
    MDD old = dst->mdd;
    dst->mdd = dddmc_ref(dddmc_union(dst->mdd, src->mdd));
    dddmc_deref(old);
    leavemt(dst);
}
...
set_minus(vset_t dst, vset_t src)
{
    ...
    assert(dst->size == src->size);
    MDD old = dst->mdd;
    dst->mdd = dddmc_ref(dddmc_minus(dst->mdd, src->mdd));
    dddmc_deref(old);
    leavemt(dst);
}
//TODO
...
set_intersect(vset_t dst, vset_t src)
{
    ...
    assert(dst->size == src->size);
    MDD old = dst->mdd;
    dst->mdd = dddmc_ref(dddmc_intersect(dst->mdd, src->mdd));
    dddmc_deref(old);
    leavemt(dst);
}
...
set_copy_match(vset_t dst, vset_t src, int p_len, int *proj, int *match)
{
    ...
    if (p_len == 0) dst->mdd = dddmc_ref(src->mdd);
    else {
        vdom_t dom = src->dom;
        int _proj[dom->shared.size+1];
        ...
        MDD mdd_proj = dddmc_ref(dddmc_cube((int32_t*)_proj, proj[p_len-1]+2, proj[p_len-1]+2));

        MDD cube = dddmc_ref(dddmc_cube((int32_t*)match, p_len, p_len));

        dst->mdd = dddmc_ref(dddmc_match(src->mdd, cube, mdd_proj));
        dddmc_deref(cube);
        dddmc_deref(mdd_proj);
    }
    leavemt(dst);
}
...
VOID_TASK_3(ddd_enumer, uint32_t*, values, size_t, count, struct enum_context*, ctx)
{
...
set_enum(vset_t set, vset_element_cb cb, void* context)
{
    ...
    dddmc_sat_all_nopar(set->mdd, (lddmc_enum_cb)TASK(ddd_enumer), set->size, &ctx);
}
...
set_project(vset_t dst, vset_t src)
{
    if (dst->proj == src->proj) {
        ...
        dddmc_deref(dst->mdd);
        dst->mdd = dddmc_ref(dddmc_project(src->mdd, dst->proj));
        leavemt(dst);
    }
}
...
set_next(vset_t dst, vset_t src, vrel_t rel)
{
    ...
    if (dst == src) {
        MDD old = dst->mdd;
        dst->mdd = dddmc_ref(dddmc_relprod(src->mdd, rel->mdd, rel->meta));
        dddmc_deref(old);
    } else {
        dddmc_deref(dst->mdd);
        dst->mdd = dddmc_ref(dddmc_relprod(src->mdd, rel->mdd, rel->meta));
...
set_prev(vset_t dst, vset_t src, vrel_t rel, vset_t universe)
{
    ...
    if (dst == src) {
        MDD old = dst->mdd;
        dst->mdd = dddmc_ref(dddmc_relprev(src->mdd, rel->mdd, rel->meta, universe->mdd));
        dddmc_deref(old);
    } else {
        dddmc_deref(dst->mdd);
        dst->mdd = dddmc_ref(dddmc_relprev(src->mdd, rel->mdd, rel->meta, universe->mdd));
...
{
    if (set->mdd == dddmc_false) Abort("set_example: empty set");
    dddmc_sat_one(set->mdd, (int32_t*)e, set->size);
}
//
//static void
...
//    leavemt(dst);
//}
//TODO vooral nog even nadenken wat met operator te doen
static void
set_dot(FILE* fp, vset_t src)
{
    dddmc_fprintdot(fp, src->mdd);
}
...
vdom_t
vdom_create_dddmc(int n, int discrete_vars)
{
    Warning(info,"Creating a multi-core DifferenceDD domain with %d discrete vars.", discrete_vars);
    ...
    dom->discrete_vars = discrete_vars;
    return dom;
}
