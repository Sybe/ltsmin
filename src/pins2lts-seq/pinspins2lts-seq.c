...
main (int argc, char *argv[])
{
    ...
    opt.model1=GBcreateBase();
    opt.model2=GBcreateBase();

    GBsetChunkMethods(opt.model1,new_string_index,NULL,
                      (int2chunk_t)HREgreyboxI2C,
                      (chunk2int_t)HREgreyboxC2I,
                      (chunkatint_t)HREgreyboxCAtI,
                      (get_count_t)HREgreyboxCount);
    Warning (info, "Loading model from %s", files[0]);
    GBloadFile(opt.model1,files[0],&opt.model1);
    Warning (info, "Model 1 read");

    GBsetChunkMethods(opt.model2,new_string_index,NULL,
                      (int2chunk_t)HREgreyboxI2C,
                      (chunk2int_t)HREgreyboxC2I,
                      (chunkatint_t)HREgreyboxCAtI,
                      (get_count_t)HREgreyboxCount);
    Warning (info, "Chunk Methods 2 set");
    Warning (info, "Loading model from %s", files[1]);
    GBloadFile(opt.model2,files[1],&opt.model2);
    Warning (info, "Model 2 read");
...
//    GBloadFile(opt.model,files[0],&opt.model);
    Warning (info, "Starting composition");
    GBparallelCompose(opt.model1, opt.model2, opt.model);
    ...
    Warning (info, "Initials state received");
    gsea_setup(files[2]);
    Warning (info, "Setup finished");
    gsea_setup_default();
    Warning (info, "Default setup finished");
    gsea_print_setup ();
    Warning (info, "Print setup finished");
    gsea_search(src);
